...
def test_version_and_svg_sprite():
    ...
    assert ('0.7.6.' in config) or ('8.0.' in config) or ('0.8.5' in config)
...
def test_service_worker_caches_svg_sprite():
    ...
    assert ("const CACHE = 'fmts-v076" in sw) or ("const CACHE = 'fmts-v80" in sw) or ("const CACHE = 'fmts-v085" in sw)
